    ...
    <div class="admin-container">
        ...
        <div class="admin-content">
            <div class="admin-header">
                <h1>Añadir Producto a Promoción</h1>
                <?php if (isset($promotion)): ?>
                <p>Promoción: <?= htmlspecialchars($promotion->getDescription()) ?> (<?= $promotion->getDiscount() ?>% de descuento)</p>
                <?php else: ?>
                <p>Error: No se ha encontrado la promoción</p>
                <?php endif; ?>
            </div>

            <?php if (isset($promotion) && isset($availableProducts) && count($availableProducts) > 0): ?>
            <div class="admin-form-container">
                <form action="/Akihabara-Dreams/promociones/productos/agregar" method="post" class="admin-form">
                    ...
                    <div class="admin-form-actions">
                        <button type="submit" class="admin-btn admin-btn-primary">Añadir a Promoción</button>
                        <a href="/Akihabara-Dreams/promociones/productos/<?= $promotion->getId() ?>" class="admin-btn admin-btn-secondary">Cancelar</a>
                    </div>
                </form>
            </div>
            <?php elseif (isset($promotion) && isset($availableProducts) && count($availableProducts) === 0): ?>
            <div class="admin-message">
                <p>No hay productos disponibles para añadir a esta promoción. Todos los productos ya están incluidos o no hay productos en el sistema.</p>
                <a href="/Akihabara-Dreams/promociones/productos/<?= $promotion->getId() ?>" class="admin-btn admin-btn-secondary">Volver</a>
            </div>
            <?php endif; ?>
        </div>
    </div>
